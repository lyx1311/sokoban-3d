...
public class FilterState extends BaseAppState {
    ...
    private List<Filter> filtersToAdd = new ArrayList<>();
    private List<Filter> filtersToRemove = new ArrayList<>();
    ...
    private void addFilters() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            // 相同滤镜只添加一次
            if (null == fpp.getFilter(filter.getClass())) fpp.addFilter(filter);
        }
    }
    ...
    @Override
    protected void onEnable() { viewPort.addProcessor(fpp); }

    @Override
    protected void onDisable() { viewPort.removeProcessor(fpp); }
}